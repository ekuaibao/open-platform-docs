...
| **&emsp; ∟ key**      | String              | 自定义字段名称 | 非必填 | - | 可通过 [获取业务对象](/docs/open-api/datalink/get-entity-list) 响应中 `items.fields.name` 获取<br/>目前支持字段值为 **文本**、**数字**、**日期** 类型字段使用<br/>例如：`id`、`ownerId`、`createTime`<br/>不支持值为 **数组** 类型字段<br/>多个参数取交集查询      |
| **&emsp; ∟ value**    | String 或<br/>Number | 查询参数 | 非必填 | - | 文本、数字类型传 **String**<br/>日期（转化为毫秒级时间戳）传 **Number**                                                                                                                                                |
...
| **400** | - | 当前页不能为0 | `index`（开始索引）不允许小于 `0` |
| **412** | - | 超过单次最大查询数量100 | `count`（查询数）不允许大于 `100` |
