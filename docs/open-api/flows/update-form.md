...
|**&emsp; &emsp; &emsp; ∟ type**                | String | 发票开票类型     | 必填  | - | `wait`:待开发票 &emsp; `exist`:已有发票 &emsp; `noExist`:无发票<br/>`noWrite`:无需填写(当费用类型发票字段设置的不可编辑时，默认为此项) |
|**&emsp; &emsp; &emsp; ∟ attachments**         | Array  | 发票附件        | 非必填 | - | 需要先通过[上传附件](/docs/open-api/attachment/attachment-upload)上传数据,然后使用接口返回值为参数 |
