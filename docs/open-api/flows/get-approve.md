...
| :--------- | :------ | :------------- |:--------- |:------ | :------------------------------------------  |
| **approverId** | String  | 员工的id |必填| - | 通过[查询员工](/corporation/get-staff-ids.html)获取 |
...
| **index**         | Number  | 分页查询的起始值        | 非必填 | 0 | 即从第几条数据查起，起始值从0开始 |
...
#### 成功响应:
可以参考[通过id集合获取单据数据列表](/flows/get-forms-sequences-ids.md)返回信息，单据数据结构是一样的
```json
{
    "items":[
        {
