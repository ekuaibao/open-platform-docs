# 根据费用类型[id]或费用类型[code]获取费用类型模板信息

此接口用于获取费用类型详情

> ⚠️ 注意：

> 费用类型id列表和费用类型code列表，至少输入一种查询条件。不允许两个查询条件都为空。

{% httpverb "post" %} /api/openapi/v2/specifications/feeType/byIdsAndCodes {% endhttpverb %}
