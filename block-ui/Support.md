...
在开始开发互联卡片之前，首先要快速熟悉下合思全局字段/单据模板的概念。

:::tip
如果你对合思单据/全局字段很熟悉，可以忽略这步
:::

合思的核心业务是依靠单据在审批流上流转来完成实际的业务需求。那么单据实体是怎么来的？
...
在自建应用创建时会自动生成应用凭证，凭证包括 App ID、App Secret 两部分。在互联卡片中请求第三方服务，或第三方服务请求合思开放接口时，都会使用 App ID、App Secret 生成 jwtToken 或者解析验证 jwtToken 是否正确。下面介绍第三方服务调用合思和合思调用第三方服务的步骤：

### 第三方服务调用合思开放接口，鉴权使用方式
