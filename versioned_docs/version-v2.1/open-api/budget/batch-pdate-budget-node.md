...
| **200** | - | 节点金额数量不匹配 | 请确认 `moneys` 字段数量与预算包设置周期数是否匹配 | 
...
| **200** | - | 节点维度设定不正确, nodeId=f0368913430d86c7b64310a79bc71f72 | 请确认新增节点中， `parentId`（父节点ID）为该 `nodeId` 的子节点的 `contentId`（维度内容ID）是否已经存在 | 
