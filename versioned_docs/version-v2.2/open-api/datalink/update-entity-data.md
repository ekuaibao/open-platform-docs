...
<details>
  ...
  [**1.17.0**](/docs/open-api/notice/update-log#1170)&emsp;-> 🐞 修复了当 `editFlag` = `increment` 时，字段传 `""` 无法清空字段值的BUG。<br/>
...
| 开关 | true |                                                                      |
| 附件 | "{\"key\": \"openapib9e51730-f778-4b81-b108-d6ca27d44ad9-登录密码.txt\",<br/> \"fileId\": \"zEIb-kGNMsoc00\", \"fileName\": \"登录密码.txt\"}" | 转义后的json格式字符串<br/>多个附件时用 `;` 分隔<br/>**`fileName` 不要包含 `;` 否则无法成功上传** |
