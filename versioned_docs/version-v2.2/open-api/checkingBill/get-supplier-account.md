...
## 成功响应
<Tabs>
<TabItem value="正常响应" label="正常响应" default>
```json
{
    "items": [
        {
            ...
            "version": 6,                //版本号
            "active": true,              //是否停用 true:启用；false:停用
            ...
            "categoryIds": [                            //品类（业务对象实体）列表
                {
                    ...
                    "fields": [                         //字段列表
                        {
                            "name": "E_ee11f575356f53e523c0_name",  //字段名称，业务对象字段唯一标识
                            "type": "text",             //类型: text：文本、date：时间、dateRange：时间范围、number：数字、money：金额、switcher：开关、ref：引用类型
                            "index": true,
                            "label": "办公用品名称",    //显示名称，账单写入时的账单抬头
                            "regex": "",
                            "source": "dataLink",
                            "formula": false,
                            "optional": false,          //是否选填，true：选填  false：必填
                            "calculation": {
                                "order": 11,
                                "dependencies": [],
                                "dependenciesBy": []
...
<TabItem value="无数据" label="无数据">

```json
{
    "items": []    //企业下没有【API写入】的供应商账户，或者【对账结算charge】未开通
}
```
</TabItem>
</Tabs>
